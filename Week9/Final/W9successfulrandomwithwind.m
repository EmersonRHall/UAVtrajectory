%% Week 9 — Robust Path-Follow + Safe Indexing (Frenet, Repulsion, Legend, Clean Plotting) + WIND (minimal-changes)
...
%% ---------------- WIND (minimal addition) ----------------
% Stationary base wind + OU gusts, fixed across space.
W_MEAN = 0.3 + 0.4*rand;                 % base speed ~[0.3..0.7] m/s (so it won't overpower)
W_DIR  = 2*pi*rand;
W_BASE = W_MEAN*[cos(W_DIR) sin(W_DIR)];
GUST_TAU = 3.0; GUST_SIGMA = 0.6;        % gentle gusts
w_gust = [0 0];                          % gust state
wind_at = @(t) (W_BASE + w_gust);        % true wind (used for sim integration)

% Compensation settings (simple & robust)
SPD_AIR_MAX_FACTOR = 1.8;                % airspeed cap vs vmax
MIN_GROUND_PROGRESS_FRAC = 0.25;         % mgp = this * vmax
% ----------------------------------------------------------

%% State + Video
pos=startXY; vel=[0 0];   % NOTE: 'vel' = AIR-RELATIVE velocity (control state)
prev_u=[1 0]; last_wp=[]; cursor_s=0; cursor_margin=0.3;
...
for k=1:Nsteps
    ...
    % Frenet path-follow + soft repulsion (unchanged)
    ...
    %% ---------- SPEED CONTROL + WIND COMPENSATION (minimal change) ----------
    % Your original distance-based speed target:
    dmin=ray_max;
    for kk=1:8
    ...
    % We want this GROUND velocity along the filtered heading:
    mgp = MIN_GROUND_PROGRESS_FRAC * vmax;           % minimum ground progress guard
    v_gnd_des = max(spd_des, mgp) * u_sm;

    % Compute the AIR command that achieves v_gnd_des in the presence of wind:
    v_air_cmd = v_gnd_des - w_vec;                   % full feed-forward (we know w_vec in sim)

    % Respect AIRSPEED limit (UAV capability)
    ...
    % Ensure the achieved ground progress (with saturated airspeed) still >= mgp
    gnd_prog = dot(v_air_cmd + w_vec, u_sm);
    if gnd_prog < mgp
        v_air_cmd = v_air_cmd + (mgp - gnd_prog)*u_sm;   % add along-track air
        na = norm(v_air_cmd);
        if na > v_air_max
            v_air_cmd = (v_air_max/na) * v_air_cmd;
        end
    ...
    % Acceleration limit (on air-relative speed, like your original code)
    dv=v_air_cmd-vel; dv_max=amax*dt; nv=norm(dv); if nv>dv_max, dv=(dv_max/nv)*dv; end
    vel=vel+dv;
    %% ------------------------------------------------------------------------

    % Integrate in substeps with slide using THICK mask
    % IMPORTANT: integrate with GROUND velocity = (air + wind)
    tryStep=(vel + w_vec)*dt; max_step=0.30*gridRes;
    Nsub=max(1,ceil(norm(tryStep)/max_step)); subStep=tryStep/Nsub;
    for ss=1:Nsub
        p_try=pos+subStep;
    ...
    % Title shows air speed (|vel|) and wind magnitude
    title(sprintf('t=%.1fs  |v_air|=%.2f  dmin=%.2f  |w|=%.2f', t, norm(vel), dmin, norm(w_vec)));
    ...
    % Wind inset (transparent, top-left — won’t hide the goal)
    axInset=axes('Position',[0.03 0.78 0.18 0.18],'Color','none','HitTest','off','PickableParts','none'); %#ok<LAXES>
    ...
    quiver(axInset,0,0,tip(1),tip(2),0,'LineWidth',1.6,'MaxHeadSize',2,'Color',[0 0 0]);
    text(axInset,-0.98,-0.98,sprintf('%.2f m/s',wmag),'Units','normalized','HorizontalAlignment','left','VerticalAlignment','bottom','FontSize',8,'FontWeight','bold','Color',[0 0 0]);
    text(axInset,0.50,0.85,'WIND','Units','normalized','HorizontalAlignment','center','FontSize',9,'FontWeight','bold','Color',[0 0 0]);
