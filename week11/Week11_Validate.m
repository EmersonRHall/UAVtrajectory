...
replans=zeros(NU,1); path_len=zeros(NU,1); min_clear_all=inf(NU,1); lowProgClock = zeros(NU,1);
...
for k=1:Nsteps
    ...
    % Rasterize other UAVs' current smoothed paths, dilate, and discourage overlap.
    ...
    for i=1:NU
        ...
        if ~isempty(smoothPath{i}) && size(smoothPath{i},1)>=2
            % remove own corridor from the mask to not penalize itself
        ...
        if t>=astar_t_next(i) || isempty(pathXY{i})
            ...
            for wi = 1:numel(weaken)
                if wi>1
                    % weaken the corridor by eroding its mask
            ...
            if ~found
                % final fallback ignoring corridors
        ...
        % -------- Wind-compensated velocity command (feed-forward + slow I) --------
        u_des = u_sm;

        % Tacking aid under strong headwind
        u_wind = w_vec / max(norm(w_vec), 1e-9);
        headwind = -dot(u_des, u_wind);
        if headwind > 0.75 && norm(w_vec) > 0.6
            g_hat = (goals(i,:) - pos(i,:)) / max(norm(goals(i,:) - pos(i,:)),1e-9);
            tR = [ u_wind(2), -u_wind(1) ];
            tL = -tR;
            cand1 = u_des + 0.55*tR;
            cand2 = u_des + 0.55*tL;
            if dot(cand2, g_hat) > dot(cand1, g_hat)
                u_des = cand2 / max(norm(cand2),1e-9);
            else
                u_des = cand1 / max(norm(cand1),1e-9);
            end
        end

        v_gnd_min = max(spd_des, MIN_GROUND_PROGRESS_FRAC * vmax * ...
                        clamp(norm(pos(i,:)-goals(i,:))/(3*tolReach),0,1));
        v_gnd_des = v_gnd_min * u_des;
        v_air_ff  = v_gnd_des - w_vec;

        A2=smoothPath{i}(max(cp_idx-1,1),:); B2=smoothPath{i}(min(cp_idx,size(smoothPath{i},1)),:);
        t_hat2=(B2-A2)/max(norm(B2-A2),1e-9);
        e_lat_vec = (pos(i,:) - cp_pt) - dot(pos(i,:)-cp_pt, t_hat2)*t_hat2;
        Kwind_I = Kwind_I_base * (1 + 0.8*min(1, norm(w_vec)/1.2));
        windInt(i,:) = 0.995*windInt(i,:) + (e_lat_vec * dt);

        v_air_cmd = v_air_ff ...
                  - 0.35 * dot(v_air_ff, n_hat) * n_hat ...
                  - Kwind_I * windInt(i,:);

        v_air_max_nom = SPD_AIR_MAX_FACTOR * vmax;
        gnd_prog_est  = dot(v_air_cmd + w_vec, u_des);
        prog_deficit  = max(0, v_gnd_min - gnd_prog_est);
        capBoost = 1.0 + 0.35 * clamp(prog_deficit/max(v_gnd_min,1e-9), 0, 1);
        v_air_max = min(1.35*v_air_max_nom, capBoost * v_air_max_nom);

        na = norm(v_air_cmd);
        if na > v_air_max, v_air_cmd = (v_air_max/na) * v_air_cmd; end

        gnd_prog = dot(v_air_cmd + w_vec, u_des);
        if gnd_prog < v_gnd_min
            v_air_cmd = v_air_cmd + min(0.9*v_air_max, v_gnd_min - gnd_prog) * u_des;
            na = norm(v_air_cmd); if na > v_air_max, v_air_cmd = (v_air_max/na) * v_air_cmd; end
        end

        if gnd_prog < 0.9*v_gnd_min
            lowProgClock(i) = lowProgClock(i) + dt;
        else
            lowProgClock(i) = max(0, lowProgClock(i) - 0.6*dt);
        end

        if lowProgClock(i) > 1.0
            v_air_cmd = v_air_cmd + WIND_THRUST_BOOST * u_des;
...
for it=1:16000
    ...
    for nb=1:8
        ...
        % Wind penalty: discourage moves into the steady wind
        step_vec = [double(neighbors(nb,2)), double(neighbors(nb,1))]; % [dx, dy]
        step_unit = step_vec / max(norm(step_vec), 1e-9);
        into_wind = max(0, dot(w_hat, step_unit));   % >0 if stepping into wind
...
[H,W] = size(freeMask);
if ~freeMask(sRC(1),sRC(2)) || ~freeMask(gRC(1),gRC(2)), ok=false; return; end
vis = false(H,W);
...
while added<nRect && attempts<400
    ...
    % ensure not overlapping with earlier rectangles
    ov=false;
...
switch lower(name)
    ...
    case 'gusty'
        ...
        MIN_GROUND_PROGRESS_FRAC=0.38;
        WIND_THRUST_BOOST=0.85;
        WIND_WEIGHT_ASTAR=0.16;
        Kwind_I=0.16;
    case 'storm'
        W_BASE=[0.90 0.25]; GUST_SIGMA=0.45; GUST_TAU=0.6;
        SPD_AIR_MAX_FACTOR=3.2;
        MIN_GROUND_PROGRESS_FRAC=0.46;
        WIND_THRUST_BOOST=1.10;
        WIND_WEIGHT_ASTAR=0.24;
        Kwind_I=0.20;
    otherwise
...
f = figure('Color','w','Position',[80 80 960 540]);
terr  = unique(T.terrain);
modesL = ["baseline","ml"];

vals = nan(numel(terr), numel(modesL));
for i = 1:numel(terr)
    for j = 1:numel(modesL)
        mask = (T.terrain==terr(i)) & (T.mode==modesL(j));
        if any(mask)
            vals(i,j) = mean(double(T.success(mask)), 'omitnan');
        end
    end
end

h = bar(vals, 'grouped'); hold on;
set(gca,'XTickLabel',cellstr(terr),'FontSize',11);
ylabel('Success rate'); title('Success by terrain'); ylim([0 1]); grid on;

for j = 1:numel(h)
    if ~(j <= size(vals,2) && ~all(isnan(vals(:,j))))
...
for j = 1:numel(h)
    x = []; y = [];
    try
        x = h(j).XEndPoints; y = h(j).YEndPoints;
    end
    if isempty(x) || isempty(y), continue; end
    for k = 1:numel(x)
        if ~isnan(y(k))
            text(x(k), y(k)+0.02, sprintf('%.0f%%', 100*y(k)), ...
                'HorizontalAlignment','center','FontSize',9);
        end
    end
end

saveas(f, fullfile(outdir,'success_by_terrain.png'));
end

function plot_box_time_len(T, outdir)
f = figure('Color','w','Position',[80 80 1080 520]);
tiledlayout(1,2,'Padding','compact','TileSpacing','compact');
nexttile; boxchart(categorical(T.mode), T.time_to_goal);
ylabel('Time (s)'); title('Time by mode');
nexttile; boxchart(categorical(T.mode), T.path_len);
...
prm.parse(varargin{:});
L  = prm.Results.longLen;
W  = prm.Results.shortLen;
...
d = pB - pA; nd = norm(d);
if nd < 1e-6
    return; % degenerate, do nothing
end
phi = atan2(d(2), d(1)) + pi/2; % perpendicular to path
mid = 0.5*(pA + pB);

% Keep a gap from the start/goal along the line to avoid touching markers
% Shift the block center along the perpendicular if needed, but first clamp
% distance from S/G along the path axis:
tA = dot(mid - pA, d)/nd; tB = dot(mid - pB, d)/nd;
if abs(tA) < cS
    mid = mid + (cS - tA) * (d/nd) * sign(tA==0) * 0; % noop if exactly centered
end
if abs(tB) < cG
    mid = mid - (cG - tB) * (d/nd) * sign(tB==0) * 0; % noop if exactly centered
end
...
if any( inside_ellipse(rect(:,1), rect(:,2), 0.10) )
    ...
    shift = 0.6 * max(rx, ry);   % push it outside the shoreline
