...
if isempty(txt)
    terrains = {'urban','natural','manmade'};
else
    s = lower(strtrim(txt));
    switch s
        case {'1','urban'}, terrains = {'urban'};
        case {'2','natural'}, terrains = {'natural'};
        case {'3','manmade','man-made','manmade'}, terrains = {'manmade'};
        otherwise, error('Unknown choice: %s', s);
    end
end
% ---------- terrain label for plots/files ----------
if numel(terrains) == 1
    terrStr = terrains{1}; 
else
    terrStr = 'all_terrains'; 
end
...
plot_success_bars(T, figdir, terrStr);
plot_box_time_len(T, figdir, terrStr);
plot_clearance_cdf(T, figdir, terrStr);
...
elseif strcmpi(terrainName,'manmade')
    minSGsep_base   = 3.5;  
    SG_MIN_CLR_base = safetyMargin + 0.20;
else
    % Urban default
    minSGsep_base   = 6.5;
    SG_MIN_CLR_base = safetyMargin + 0.60;
end
...
elseif strcmpi(terrainName,'manmade')
    cornerBoxes = [ ...
        -2,  xyMin(2)+2,              3.5, 3.5;  
         3,  xyMin(2)+2,              3.5, 3.5; 
        -2,  xyMax(2)-2-3.5,          3.5, 3.5; 
         3,  xyMax(2)-2-3.5,          3.5, 3.5];
end
...
% === HARD OVERRIDE for manmade BLUE UAV (U1) ===
if strcmpi(terrainName,'manmade')
    ...
    if numel(idxS) >= 1 && numel(idxG) >= 1
        ...
        if has_path_bfs(sRC, gRC, ~occ_static)
            ...
            warning('manmade override for U1: chosen start/goal not connected, keeping random S/G.');
        end
    else
        warning('manmade override for U1: not enough valid cells in start/goal bands, keeping random S/G.');
    end
end

% === HARD OVERRIDE for URBAN — 3 long, spread-out routes on the roads ===
if strcmpi(terrainName,'urban') && NU >= 3
    ...
    xRoadCenter = xL + (cxRoadIdx-0.5)*blockW;
    yRoadCenter = yB + (cyRoadIdx-0.5)*blockH;

    edgePadRoad = 1.2;
    % U1 (blue): LONG vertical run on main N–S road
    starts(1,:) = [xRoadCenter + 8, yB + edgePadRoad]; 
    goals(1,:)  = [xRoadCenter, yT - edgePadRoad];

    % U2 (red): LONG horizontal run on main E–W road
    starts(2,:) = [xL + edgePadRoad, yRoadCenter]; 
    goals(2,:)  = [xR - edgePadRoad, yRoadCenter + 8];

    % U3 (yellow): diagonal-ish path using both roads
    % Start high on vertical road, end rightward on horizontal road
    starts(3,:) = [xRoadCenter - 1.5, yRoadCenter + 2*blockH];
    goals(3,:)  = [xRoadCenter + 2*blockW,  yRoadCenter + 2];

end


% --- Inject an obstacle between the YELLOW UAV's start and goal ---
...
S.pondInfo = pondInfo;
S.terrainName = string(terrainName);
end
...
INT_MAX_STEP = 0.08*gridRes;
RAY_STEP     = 0.18*gridRes;
...
for k=1:Nsteps
    ...
    terrainName = lower(string(terrain_of(S)));
    corridor_rad = 0.8;  

    if terrainName == "urban"
        corridor_rad = 1.5;  
    elseif terrainName == "manmade"
        corridor_rad = 1.1; 
    elseif terrainName == "natural"
        corridor_rad = 0.9;    
    end
    ...
    % Draw visual roads in URBAN only (UAVs can fly over; not obstacles)
    if lower(string(terrain_of(S))) == "urban"
        draw_urban_roads(axMain, xyMin, xyMax);
    end
    ...
    % --- Draw paths and trajectories (NO legend handles here) ---
    for i=1:opts.NU
        % A* path (grey dashed)
        if ~isempty(pathXY{i}) && size(pathXY{i},1)>1
            plot(axMain, pathXY{i}(:,1),pathXY{i}(:,2),'--', ...
                'Color',[0.6 0.6 0.6],'LineWidth',1.2);
        end

        % Smoothed path (dark grey)
        if ~isempty(smoothPath{i}) && size(smoothPath{i},1)>1
            plot(axMain, smoothPath{i}(:,1),smoothPath{i}(:,2),'-', ...
                'Color',[0.35 0.35 0.35],'LineWidth',1.8);
        end

        % UAV traj + start + goal
        col = colors(i,:);
        plot(axMain, traj(1:k,1,i),traj(1:k,2,i),'-','Color',col,'LineWidth',2.0);
        plot(axMain, starts(i,1),starts(i,2),'o','MarkerFaceColor',col,'Color',col,'MarkerSize',6);
        plot(axMain, goals(i,1),goals(i,2),'x','Color',col,'LineWidth',1.6,'MarkerSize',10);
    end

    % --- Compact legend: only 3 UAVs ---
    legend(axMain,'off'); 

    LEG_COLORS = colors; 
    h1 = plot(axMain, nan, nan, '-', 'Color', LEG_COLORS(1,:), 'LineWidth', 2);
    h2 = plot(axMain, nan, nan, '-', 'Color', LEG_COLORS(2,:), 'LineWidth', 2);
    h3 = plot(axMain, nan, nan, '-', 'Color', LEG_COLORS(3,:), 'LineWidth', 2);

    lg = legend(axMain, [h1 h2 h3], ...
        {'Blue UAV','Red UAV','Yellow UAV'}, ...
        'Location','southoutside', ...
        'Orientation','horizontal', ...
        'FontSize',8, ...
        'AutoUpdate','off');

    WIND_DISPLAY_SCALE = 25; 
    wmag_sim  = norm(w_vec);
    wmag_real = WIND_DISPLAY_SCALE * wmag_sim;

    title(axMain, sprintf('Week 11 (%s/%s/%s) | t=%.1fs | |wind|=%.1f m/s', ...
        upper(string(terrain_of(S))), upper(string(weatherStr)), upper(string(modeStr)), ...
        (k-1)*dt, wmag_real));
    ...
    if ~isempty(vw) && isvalid(vw)
        try
            % Capture the entire figure (axes + title + wind inset)
            fr = getframe(fig);
            [C,~] = frame2im(fr);
...
for it=1:16000
    ...
    for nb=1:8
        ...
        step_vec = [double(neighbors(nb,2)), double(neighbors(nb,1))]; 
...
% --- SPECIAL CASE: manmade "harbor" layout ---
if ~isempty(staticPolys)
    H = staticPolys{1};
    xs = H(:,1); ys = H(:,2);

    looksLikeHarbor = ...
        (max(xs) - min(xs)) < 0.6*span(1) && ...     
        min(xs) <= xyMin(1) + 0.08*span(1) && ...    
        max(xs) >= xyMin(1) + 0.25*span(1) && ... 
        min(ys) <= xyMin(2) + 0.05*span(2) && ... 
        max(ys) >= xyMax(2) - 0.05*span(2);  

    if looksLikeHarbor
        % Man-made harbor is tighter
        maxSamples = 40000; 
        edgePad    = 1.0; 
        minPairDist= 3.0;
    end
...
switch name
    ...
    case "manmade", [staticPolys, polyColors, pondInfo] = make_manmade(xyMin, xyMax);
    otherwise, error('Unknown terrain "%s".', name);
end
end

function [P,C,pondInfo] = make_urban(xyMin, xyMax)
% Structured urban terrain:
...
% Number of blocks in x/y
nx = 4;
ny = 4;

% Size of each block region
blockW = span(1) / nx;
blockH = span(2) / ny;

% Fraction of each block reserved as ROAD around buildings
roadFracX = 0.35; 
roadFracY = 0.35;
...
for ix = 1:nx
    for iy = 1:ny
        ...
        % Building footprint size inside block (leave margins as road)
        bW = blockW * (1 - roadFracX);
        bH = blockH * (1 - roadFracY);

        % Simple axis-aligned rectangle building
        rect = oriented_rect([cx cy], bW, bH, 0); 
        rect = [rect; rect(1,:)]; 
        P{end+1} = rect;
        C{end+1} = [0.45 0.45 0.5]; 
    end
end
...
% Natural: central pond; keep all other features OUT of pond
P={}; C={};
...
while added<nRect && attempts<400
    ...
    rectShapes{end+1}=ps; rectColors{end+1}=[0.6 0.6 0.7];
    added=added+1;
end
for q=1:numel(rectShapes)
...
%% Warehouse blocks on the RIGHT
padOuterX = 0.12 * span(1);
padOuterY = 0.12 * span(2);

xStart = xL + harborWidth + 0.40*span(1); 
xEnd   = xR - padOuterX;
yStart = yB + padOuterY;
yEnd   = yT - padOuterY;
...
switch lower(name)
    ...
    case 'gusty'
        ...
        MIN_GROUND_PROGRESS_FRAC=0.36;  
        WIND_THRUST_BOOST=0.95;         
        WIND_WEIGHT_ASTAR=0.00;
        Kwind_I=0.18;
    case 'storm'
        W_BASE=[0.90 0.25]; GUST_SIGMA=0.45; GUST_TAU=0.6;
        SPD_AIR_MAX_FACTOR=3.4;         
        MIN_GROUND_PROGRESS_FRAC=0.44;  
        WIND_THRUST_BOOST=1.25;        
        WIND_WEIGHT_ASTAR=0.00;         
        Kwind_I=0.22;                   
    otherwise
...
function plot_success_bars(T, outdir, terrStr)
% Makes TWO figures:
[BASELINE_COLOR, ML_COLOR] = mode_colors();

terr = unique(T.terrain,'stable');
...
saveas(f1, fullfile(outdir, sprintf('success_by_%s_baseline.png', terrStr)));
...
saveas(f2, fullfile(outdir, sprintf('success_by_%s_ml.png', terrStr)));
end


function plot_box_time_len(T, outdir, terrStr)
[BASELINE_COLOR, ML_COLOR] = mode_colors();
...
% ---- Time by mode ----
nexttile; hold on;
maskB = (T.mode=="baseline");
maskM = (T.mode=="machine learning");

if any(maskB)
    bcB = boxchart(categorical(modePretty(maskB)), T.time_to_goal(maskB));
    bcB.BoxFaceColor = BASELINE_COLOR;
end
if any(maskM)
    bcM = boxchart(categorical(modePretty(maskM)), T.time_to_goal(maskM));
    bcM.BoxFaceColor = ML_COLOR;
end
ylabel('Time (s)');
title(sprintf('Time by Mode - %s', terrStr));
grid on;

% ---- Path length by mode ----
nexttile; hold on;
if any(maskB)
    bpB = boxchart(categorical(modePretty(maskB)), T.path_len(maskB));
    bpB.BoxFaceColor = BASELINE_COLOR;
end
if any(maskM)
    bpM = boxchart(categorical(modePretty(maskM)), T.path_len(maskM));
    bpM.BoxFaceColor = ML_COLOR;
end
ylabel('Path Length (m)');
title(sprintf('Path Length by Mode - %s', terrStr));
grid on;

saveas(f, fullfile(outdir, sprintf('time_len_by_mode_%s.png', terrStr)));
end


function plot_clearance_cdf(T, outdir, terrStr)
[BASELINE_COLOR, ML_COLOR] = mode_colors();
...
title(sprintf('Clearance CDF - %s', terrStr));

saveas(f, fullfile(outdir, sprintf('clearance_cdf_%s.png', terrStr)));
...
function draw_urban_roads(ax, xyMin, xyMax)
% Draw a cross-shaped road in the city
% Purely visual (NOT obstacles).
...
% Road widths
vertWidth   = 0.40 * span(1) / 4;   % vertical road total width
horizWidth  = vertWidth;            % same for horizontal
roadColor   = [0.80 0.80 0.75];     % light concrete color

% Shifted centers
xCenter = 0.5*(xL + xR) - 2.5;
yCenter = 0.5*(yB + yT) - 2.5;

%% --- Main road patches ---

% Vertical road (north–south)
vert = [ xCenter-vertWidth/2  yB;
         xCenter+vertWidth/2  yB;
         xCenter+vertWidth/2  yT;
         xCenter-vertWidth/2  yT ];

patch('Parent',ax, ...
      'XData',vert(:,1),'YData',vert(:,2), ...
      'FaceColor',roadColor, ...
      'FaceAlpha',0.7, ...
      'EdgeColor','none');

% Horizontal road (east–west)
horiz = [ xL  yCenter-horizWidth/2;
          xR  yCenter-horizWidth/2;
          xR  yCenter+horizWidth/2;
          xL  yCenter+horizWidth/2 ];

patch('Parent',ax, ...
      'XData',horiz(:,1),'YData',horiz(:,2), ...
      'FaceColor',roadColor, ...
      'FaceAlpha',0.7, ...
      'EdgeColor','none');
...
lineColor   = [0.95 0.85 0.10];  % bright yellow
lineWidthFr = 0.08;              % fraction of road width for each stripe
gapFr       = 0.08;              % gap between the two yellow lines

% Vertical double yellow (runs along y, split in x)
centerGap   = gapFr * vertWidth;
stripeWidth = lineWidthFr * vertWidth;

xLeftStripe  = xCenter - centerGap/2 - stripeWidth/2;
xRightStripe = xCenter + centerGap/2 + stripeWidth/2;

vLeft = [ xLeftStripe - stripeWidth/2   yB;
          xLeftStripe + stripeWidth/2   yB;
          xLeftStripe + stripeWidth/2   yT;
          xLeftStripe - stripeWidth/2   yT ];

vRight = [ xRightStripe - stripeWidth/2   yB;
           xRightStripe + stripeWidth/2   yB;
           xRightStripe + stripeWidth/2   yT;
           xRightStripe - stripeWidth/2   yT ];

patch('Parent',ax, ...
      'XData',vLeft(:,1),'YData',vLeft(:,2), ...
      'FaceColor',lineColor, ...
      'FaceAlpha',1.0, ...
      'EdgeColor','none');

patch('Parent',ax, ...
      'XData',vRight(:,1),'YData',vRight(:,2), ...
      'FaceColor',lineColor, ...
      'FaceAlpha',1.0, ...
      'EdgeColor','none');

% Horizontal double yellow (runs along x, split in y)
centerGapH   = gapFr * horizWidth;
stripeWidthH = lineWidthFr * horizWidth;

yLowerStripe = yCenter - centerGapH/2 - stripeWidthH/2;
yUpperStripe = yCenter + centerGapH/2 + stripeWidthH/2;

hLow = [ xL  yLowerStripe - stripeWidthH/2;
         xR  yLowerStripe - stripeWidthH/2;
         xR  yLowerStripe + stripeWidthH/2;
         xL  yLowerStripe + stripeWidthH/2 ];

hHigh = [ xL  yUpperStripe - stripeWidthH/2;
          xR  yUpperStripe - stripeWidthH/2;
          xR  yUpperStripe + stripeWidthH/2;
          xL  yUpperStripe + stripeWidthH/2 ];

patch('Parent',ax, ...
      'XData',hLow(:,1),'YData',hLow(:,2), ...
      'FaceColor',lineColor, ...
      'FaceAlpha',1.0, ...
      'EdgeColor','none');

patch('Parent',ax, ...
      'XData',hHigh(:,1),'YData',hHigh(:,2), ...
      'FaceColor',lineColor, ...
      'FaceAlpha',1.0, ...
      'EdgeColor','none');
end
...
% Use sim magnitude for arrow length (visual) but clamp as before
L = 0.42 * s * min(1, wmag_sim/2.0);

quiver(ax, xc, yc, L*dir(1), L*dir(2), 0, ...
       'LineWidth',1.6,'MaxHeadSize',2,'Color',[0 0 0]);

text(ax, x0+0.04*s, y0+0.04*s, sprintf('%.1f m/s', wmag_real), ...
...
function name = terrain_of(S)
if isfield(S,'terrainName') && ~isempty(S.terrainName)
    name = string(S.terrainName);
else
    name = "scenario";
end
end
...
cS = prm.Results.clearStart; 
cG = prm.Results.clearGoal;  
...
% Final: append polygon and color
