%% Week 9 — Integration with Simulation (robust descent + shifted dynamic obstacle)
% Hybrid: A* + smoothing + pure pursuit + ML nudge
...
% Gray box (true axis-aligned box)
...
%% --- Dynamic obstacle (shifted left & up) + wind ---
dyn_x_shift = -0.6;  % negative = left
dyn_y_shift =  1.2;  % positive = up
dynBase=[-1.8+dyn_x_shift  -6.2+dyn_y_shift; 
          1.8+dyn_x_shift  -6.2+dyn_y_shift; 
          1.8+dyn_x_shift  -3.8+dyn_y_shift; 
         -1.8+dyn_x_shift  -3.8+dyn_y_shift];
dynAmp = 2.0;            % horizontal sweep amplitude (±)
dynPer = 8.0;            % seconds per sweep
wind_xy=[0.20 -0.10]; 
wind_sigma=0.04;
...
% Downward bias fallback (prevents up-hover when goal is below)
DOWNLOCK_ON = true; DOWNLOCK_GAIN = 0.78;

% Lip penalty band (discourage skimming at wall top)
...
for k=1:Nsteps
    ...
    %% ------------ DESCENT engage (L-path LoS; fallback if helper missing) ------------
    ...
    % Downward intent bias when above wall & goal is below
...
function tf = isLineFree(pA, pB, isOcc, step, min_clearance)
%ISLINEFREE  Sample the segment pA->pB and test occupancy (with padding).
%   tf = isLineFree(pA, pB, isOcc, step)
%   tf = isLineFree(pA, pB, isOcc, step, min_clearance)
    if nargin < 5, min_clearance = 0; end
    seg = pB - pA;
    L = norm(seg);
    if L < 1e-9
        tf = ~isOcc(pA);
    ...
    nsteps = max(2, ceil(L / max(step, 1e-6)));
    tf = true;
    for s = 1:nsteps
        q = pA + (s/nsteps) * seg;
        if isOcc(q) || isOcc(q+[min_clearance 0]) || isOcc(q+[-min_clearance 0]) || ...
